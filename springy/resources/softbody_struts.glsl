...
void main(){
    ...
    if (strut.f1 != -1 && strut.f2 != -1) {
        vec3 _nl = cross(x1-x0, x2-x0);
        vec3 _nr = cross(x3-x0, x1- x0);
        if (length(x1-x0) > 0.0001 && length(x2-x0) > 0.0001 && length(x3-x0) > 0.0001 && length(_nl) > 0.0001 && length(_nr) > 0.0001 ) {
            vec3 nl = normalize(_nl);
            vec3 nr = normalize(_nr);
