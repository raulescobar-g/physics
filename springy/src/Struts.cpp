...
int Struts::get_poly_count() {
    return triangle_count;
}

void Struts::load_struts_mesh() {
    GLfloat temp[12] = {
        -0.5f, -0.5f, 0.0f,
        0.5f, -0.5f, 0.0f,
        -0.5f, 0.5f, 0.0f,
...
void Struts::buffer_world_geometry(const std::vector<std::shared_ptr<Object> >& objects) {
    if (objects.size() > 0) {
        // compute stuff
...
glm::vec4 Struts::get_particle_count_data() {
...
void Struts::init(int max, int spawns, int start, const std::shared_ptr<Program> compute_program) {
    max_amount = max;
    initial = start;
    current = 0;
...
void Struts::draw(const std::shared_ptr<Program> particle_render_program, const std::shared_ptr<Program> compute_program) const {
