...
void extract_triangles(std::shared_ptr<Object> obj, std::vector<triangle>& triangles, std::vector<data>& amount_of_triangles) {
    ...
    MV->scale(obj->scale);
    if (glm::length(obj->rotation) > 0.0001) MV->rotate(glm::length(obj->rotation), obj->rotation);
...
void Particles::buffer_world_geometry(const std::vector<std::shared_ptr<Object> >& objects) {
    ...
    for (auto obj : objects) {
        ...
        if (glm::length(obj->rotation) > 0.001f) MV *= glm::rotate(glm::mat4(1.0f), glm::length(obj->rotation), obj->rotation);
...
void Particles::init(int max, const std::shared_ptr<Program> compute_program) {
    max_amount = max;
    amount = std::min(max, 1024 * 1024);
